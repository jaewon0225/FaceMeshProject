...
class deformer():
    ...
    def deform_image(self, q):
        Img_map = np.zeros((self.img.shape[0], self.img.shape[1], 2), dtype=np.float32)
        new_v = calculate_new_v(q, self.A_list, self.w, self.w_sum, self.v)
        k = 0
        for j in enumerate(Img_map[:, 0, 0]):
            for i in enumerate(Img_map[0, :, 0]):
                Img_map[i[0], j[0], 0] = new_v[k][0]
                Img_map[i[0], j[0], 1] = new_v[k][1]
                k += 1
        dst_img = cv2.remap(self.img, invert_map(Img_map)[:, :, 0].reshape(self.img.shape[0], self.img.shape[1]),
                            invert_map(Img_map)[:,:,1].reshape(self.img.shape[0], self.img.shape[1]),
                            cv2.INTER_LINEAR)
        return dst_img
